...
public class Mission {
    ...
    @Builder.Default
    @Enumerated(EnumType.STRING)
    @Column(name = "mission_type", length = 10)
    private MissionType missionType = MissionType.NORMAL;

    @Builder.Default
    @Column(name = "express_surcharge", precision = 5, scale = 3)
    ...
    public enum CargoType      { GENERAL, DANGEROUS, PERISHABLE, OVERSIZED, LIQUID, CONTAINER,
                                  AGRICULTURAL, LIVESTOCK, COFFEE_COCOA, CEREALS, TIMBER, FERTILIZER }
    public enum MissionType    { NORMAL, EXPRESS }
}
